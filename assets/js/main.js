		...
		$('.gallery')
			.on('click', 'a', function(event) {
				...
				var $a = $(this),
					...
					$modalCaption = $modal.find('.caption'),
					href = $a.attr('href'),
					titleText = $a.attr('data-title') || '';

				// Not an image? Bail.
					if (!href.match(/\.(jpg|gif|png|jpeg|webp|mp4)$/i))
						return;
				...
				// Set caption (project title / description).
					if ($modalCaption.length)
						$modalCaption.text(titleText).toggle(!!titleText);
			...
			.prepend('<div class="modal" tabIndex="-1"><div class="inner"><img src="" /><div class="caption"></div></div></div>')
				.find('img')
